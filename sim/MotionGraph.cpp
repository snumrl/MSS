...
MotionGraph(const dart::dynamics::SkeletonPtr& skel,std::map<std::string,std::string> bvh_map,double time_step)
	:mSkeleton(skel),mBVHMap(bvh_map),mTimeStep(time_step),mTotalElapsedTime(0.0),mElapsedTime(0.0),mCurrentSequence(-1),mCurrentRootOffset(Eigen::Vector6d::Zero()),md(Eigen::VectorXd::Zero(skel->getNumDofs())),mt_s(0.5),mRtarget(Eigen::Matrix3d::Identity()),mRy(Eigen::Matrix3d::Identity())
{
